...
namespace Infraestructure.Command
{
    public class StudyCommand : IStudyCommand
    {
        ...
        public async Task<Study> UpdateStudy(int StudyId, StudyRequest request)
        {
            try
            {
                var studyUpdate = await _context.studys
                    .Include(r => r.resume)
                    .Include(st => st.studyType)
                    .FirstOrDefaultAsync(s => s.StudyId == StudyId && s.UserId == request.UserId);

                if (studyUpdate== null)
                {
                    throw new ExceptionNotFound("El user con id" +request.UserId+ "no existe");
                }
                studyUpdate.UserId = request.UserId;
